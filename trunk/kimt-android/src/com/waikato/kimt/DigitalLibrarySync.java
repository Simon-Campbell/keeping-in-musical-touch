...
public interface DigitalLibrarySync {
	...
	/**
	 * Returns the most up-to-date version of a MusicLibrary
	 * object from the Keeping In Musical Touch syncher.
	 * @return
	 * 	An up-to-date MusicLibrary object containing the current
	 * 	state of the project.
	 */
	MusicLibrary	getRemoteLibrary();
	
	/**
	 * Set the library in the library syncher if this device
	 * has permissions to do so (decided by remote device) 
	 * @param ml
	 * 	The new music library for other devices to set.
	 * @return
	 * 	A boolean indicating whether the process was successful 
	 *  or not.
	 */
	boolean			setRemoteLibrary(MusicLibrary ml);
}
